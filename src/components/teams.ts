	...
	server.tool(
		...
		'Create a new Team under your account',
		{
			...
			name: z
				...
				.max(256)
				.optional()
				.describe('The desired name for the Team'),
			attribution: z
				.object({
	...
	server.tool(
		...
		'Delete a team under your account',
		{
			...
			reasons: z
				...
				.optional()
				.describe('Reasons for team deletion'),
		},
	...
	server.tool(
		...
		"Update a team member's role or status",
		{
			...
			confirmed: z
				.literal(true)
				.optional()
				.describe("Accept user's request to join"),
			role: z
				.enum(['MEMBER', 'VIEWER'])
				.optional()
			...
			joinedFrom: z
				...
				.optional()
				.describe('SSO connection information'),
		},
		async ({ teamId, uid, confirmed, role, projects, joinedFrom }) => {
			const response = await fetch(
				`${BASE_URL}/v1/teams/${teamId}/members/${uid}`,
				{
					method: 'PATCH',
					headers: {
						'Content-Type': 'application/json',
	...
	server.tool(
		...
		'Update team information',
		{
			...
			avatar: z
				...
				.regex(/^[a-f0-9]+$/)
				.optional()
				.describe('Hash of uploaded image'),
			previewDeploymentSuffix: z
				.string()
				.nullable()
				.optional()
				.describe('Preview deployment suffix'),
			emailDomain: z
				.union([
				...
				.optional()
				.describe('Team email domain'),
			regenerateInviteCode: z
				.boolean()
				.optional()
				.describe('Create new invite code'),
			saml: z
				.object({
					enforced: z.boolean(),
			...
			enablePreviewFeedback: z
				.enum(['on', 'off', 'default'])
				.optional()
				.describe('Preview toolbar setting'),
			enableProductionFeedback: z
				.enum(['on', 'off', 'default'])
				.optional()
				.describe('Production toolbar setting'),
			sensitiveEnvironmentVariablePolicy: z
				.enum(['on', 'off', 'default'])
				.optional()
				.describe('Sensitive env var policy'),
			remoteCaching: z
				.object({
					enabled: z.boolean(),
				})
			...
			hideIpAddresses: z
				.boolean()
				.optional()
				.describe('Hide IP addresses in monitoring'),
			hideIpAddressesInLogDrains: z
				.boolean()
				.optional()
				.describe('Hide IP addresses in log drains'),
		},
