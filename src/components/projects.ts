...
export function registerProjectTools(server: McpServer, accessToken: string) {
	// List all projects
	server.tool(
		'list_projects',
	...
	server.tool(
		...
		'Create a new project with the provided configuration',
		{
			...
			gitRepository: z
				...
				.optional()
				.describe('The Git Repository to connect'),
			environmentVariables: z
				...
				.optional()
				.describe('Collection of ENV Variables'),
	...
	server.tool(
		...
		async ({ idOrName, teamId, slug }) => {
			...
			if (response.status === 204) {
				return {
					content: [
						{
							type: 'text',
							text: `Project ${idOrName} was successfully deleted`,
						},
					],
				};
			}
			...
			throw new Error(
				`Failed to delete project: ${response.status} - ${errorData}`,
			);
		},
	);
	...
	server.tool(
		...
		async ({ idOrName, domain, teamId, slug }) => {
			const url = new URL(
				`${BASE_URL}/v9/projects/${idOrName}/domains/${domain}`,
			);
			if (teamId) url.searchParams.append('teamId', teamId);
			if (slug) url.searchParams.append('slug', slug);
	...
	server.tool(
		...
		'List deployments for a project',
		{
			projectId: z
				.string()
				.optional()
				.describe('Filter deployments from the given project ID'),
			...
			target: z
				.string()
				.optional()
				.describe('Filter deployments based on environment'),
			users: z
				.string()
				.optional()
				.describe('Filter deployments by user IDs (comma-separated)'),
			rollbackCandidate: z
				.boolean()
				.optional()
		...
		async ({ idOrName, env, teamId, slug }) => {
			...
			return {
				content: [
					{
						type: 'text',
						text: `Environment variables added:\n${JSON.stringify(
							data,
							null,
							2,
						)}`,
		...
		async ({ idOrName, envId, value, type, target, teamId, slug }) => {
			...
			return {
				content: [
					{
						type: 'text',
						text: `Environment variable updated:\n${JSON.stringify(
							data,
							null,
							2,
						)}`,
		...
		async ({ idOrName, envId, teamId, slug }) => {
			...
			return {
				content: [
					{
						type: 'text',
						text: `Environment variable deleted:\n${JSON.stringify(
							data,
							null,
							2,
						)}`,
		...
		async ({ idOrName, envId, teamId, slug }) => {
			...
			return {
				content: [
					{
						type: 'text',
						text: `Environment variable details:\n${JSON.stringify(
							data,
							null,
							2,
						)}`,
	...
	server.tool(
		...
		async ({ projectId, deploymentId, teamId, slug }) => {
			const url = new URL(
				`${BASE_URL}/v10/projects/${projectId}/promote/${deploymentId}`,
			);
			if (teamId) url.searchParams.append('teamId', teamId);
			if (slug) url.searchParams.append('slug', slug);
	...
	server.tool(
		...
		async ({ projectId, teamId, slug }) => {
			const url = new URL(
				`${BASE_URL}/v1/projects/${projectId}/promote/aliases`,
			);
			if (teamId) url.searchParams.append('teamId', teamId);
			if (slug) url.searchParams.append('slug', slug);
	...
	server.tool(
		...
		async ({ idOrName, teamId }) => {
			const response = await fetch(
				`${BASE_URL}/projects/${idOrName}/transfer-request`,
				{
					method: 'POST',
					headers: {
						'Content-Type': 'application/json',
	...
	server.tool(
		...
		async ({ code }) => {
			const response = await fetch(
				`${BASE_URL}/projects/transfer-request/${code}`,
				{
					method: 'PUT',
					headers: {
						Authorization: `Bearer ${accessToken}`,
