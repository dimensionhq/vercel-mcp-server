...
export function registerArtifactTools(server: McpServer, accessToken: string) {
	...
	server.tool(
		...
		'Downloads a cache artifact identified by its hash',
		{
			...
			ci: z
				.string()
				.optional()
				.describe('The CI environment where this artifact is downloaded'),
			interactive: z
				.boolean()
				.optional()
				.describe('Whether the client is an interactive shell'),
		},
		async ({ hash, teamId, slug, ci, interactive }) => {
			...
			return {
				content: [
					{ type: 'text', text: `Artifact downloaded: ${data.size} bytes` },
				],
			};
		},
	);
	...
	server.tool(
		...
		'Records artifacts cache usage events',
		{
			...
			ci: z
				.string()
				.optional()
				.describe('The CI environment where events occurred'),
			interactive: z
				.boolean()
				.optional()
				.describe('Whether the client is an interactive shell'),
	...
	server.tool(
		...
		'Uploads a cache artifact identified by its hash',
		{
			...
			duration: z
				.number()
				.optional()
				.describe('Time taken to generate the artifact in milliseconds'),
			ci: z
				.string()
				.optional()
				.describe('The CI environment where this artifact was generated'),
			interactive: z
				.boolean()
				.optional()
				.describe('Whether the client is an interactive shell'),
			tag: z
				.string()
				.optional()
				.describe('Base64 encoded tag for this artifact'),
			teamId: z
				.string()
				.optional()
				.describe('The Team identifier to perform the request on behalf of'),
