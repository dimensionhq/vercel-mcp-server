...
export function registerSecretTools(server: McpServer, accessToken: string) {
	// Create a new secret
	server.tool(
		'create_secret',
		'Create a new secret',
		{
			name: z
				.string()
				.max(100)
				.describe('The name of the secret (max 100 characters)'),
			value: z.string().describe('The value of the new secret'),
			decryptable: z
				.boolean()
				.optional()
				.describe('Whether the secret value can be decrypted after creation'),
			projectId: z
				.string()
				.optional()
				.describe('Associate a secret to a project'),
			...teamParams,
		},
	...
	server.tool(
		...
		'List all secrets',
		{
			id: z
				.string()
				.optional()
				.describe('Filter by comma separated secret ids'),
