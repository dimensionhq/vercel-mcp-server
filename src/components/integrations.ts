...
export function registerIntegrationTools(
		...
		async ({ id, teamId, slug }) => {
			...
			return {
				content: [
					{
						type: 'text',
						text: 'Integration configuration deleted successfully',
					},
				],
			};
		},
	...
	server.tool(
		...
		'Get configurations for the authenticated user or team',
		{
			view: z
				...
				.describe('View type for configurations'),
			installationType: z
				.enum(['marketplace', 'external'])
				.optional()
				.describe('Type of installation'),
			integrationIdOrSlug: z
				.string()
				.optional()
	...
	server.tool(
		...
		'List git namespaces by provider',
		{
			host: z
				.string()
				.optional()
				.describe('The custom Git host if using a custom Git provider'),
			provider: z
				.enum(['github', 'github-custom-host', 'gitlab', 'bitbucket'])
				.optional()
	...
	server.tool(
		...
		async ({
			...
			return {
				content: [
					{ type: 'text', text: 'Integration action updated successfully' },
				],
			};
		},
	);
