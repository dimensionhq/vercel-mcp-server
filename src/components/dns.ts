...
export function registerDnsTools(server: McpServer, accessToken: string) {
	// Create DNS Record
	server.tool(
		'create_dns_record',
		'Creates a DNS record for a domain',
		{
			...
			comment: z
				...
				.max(500)
				.optional()
				.describe('A comment to add context'),
			teamId: z
				.string()
				.optional()
	...
	server.tool(
		...
		async ({ domain, recordId, teamId, slug }) => {
			const url = new URL(
				`${BASE_URL}/v2/domains/${domain}/records/${recordId}`,
			);
			if (teamId) url.searchParams.append('teamId', teamId);
			if (slug) url.searchParams.append('slug', slug);
	...
	server.tool(
		...
		'Retrieves a list of DNS records created for a domain name',
		{
			...
			limit: z
				.string()
				.optional()
				.describe('Maximum number of records to list'),
			since: z
				.string()
				.optional()
	...
	server.tool(
		...
		'Updates an existing DNS record for a domain name',
		{
			...
			name: z
				...
				.nullable()
				.optional()
				.describe('The name of the DNS record'),
			value: z
				.string()
				.nullable()
				.optional()
				.describe('The value of the DNS record'),
			type: z
				.enum(['A', 'AAAA', 'ALIAS', 'CAA', 'CNAME'])
				.nullable()
				.optional()
				.describe('The type of the DNS record'),
			ttl: z
				.number()
				.min(60)
				.max(2147483647)
			...
			mxPriority: z
				...
				.nullable()
				.optional()
				.describe('The MX priority value'),
			comment: z
				.string()
				.max(500)
				.optional()
