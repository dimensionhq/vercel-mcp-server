...
export function registerSecurityTools(server: McpServer, accessToken: string) {
	...
	server.tool(
		...
		'Update the Attack Challenge mode settings',
		{
			...
			attackModeActiveUntil: z
				.number()
				.nullable()
				.optional()
				.describe('Timestamp until attack mode is active'),
			...teamParams,
		},
		async ({
			projectId,
	...
	server.tool(
		...
		async ({ projectId, configVersion, teamId, slug }) => {
			const url = new URL(
				`${BASE_URL}/v1/security/firewall/config${
		...
		async ({ projectId, teamId, slug, ...updateData }) => {
			...
			return {
				content: [
					{
						type: 'text',
						text: `Firewall configuration updated:\n${JSON.stringify(
							data,
							null,
							2,
						)}`,
	...
	server.tool(
		...
		'Set the complete firewall configuration',
		{
			...
			managedRules: z
				...
				.optional()
				.describe('Managed rules configuration'),
			crs: z
				.object({
					sd: z
						.object({ active: z.boolean(), action: z.enum(['deny']) })
						.optional(),
					ma: z
						.object({ active: z.boolean(), action: z.enum(['deny']) })
						.optional(),
				...
				.optional()
				.describe('Custom rule set configuration'),
			rules: z
				.array(
					z.object({
						id: z.string(),
		...
		async ({ projectId, teamId, slug, ...config }) => {
			...
			return {
				content: [
					{
						type: 'text',
						text: `Firewall configuration set:\n${JSON.stringify(
							data,
							null,
							2,
						)}`,
