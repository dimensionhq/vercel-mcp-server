...
export function registerAliasTools(server: McpServer, accessToken: string) {
	// Assign an Alias
	server.tool(
		'assign_alias',
		'Creates a new alias for the deployment with the given deployment ID',
		{
			id: z
				.string()
				.describe('The ID of the deployment to assign the alias to'),
			alias: z
				.string()
				.describe('The alias to assign (e.g. my-alias.vercel.app)'),
			redirect: z
				.string()
				.nullable()
				.optional()
				.describe('Optional hostname to redirect to using 307'),
			teamId: z
				.string()
				.optional()
